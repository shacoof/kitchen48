/**
 * FavoritesPage
 * Shows the current user's saved (bookmarked) recipes.
...
export function FavoritesPage() {
  ...
  useEffect(() => {
    ...
    const fetchSaved = async () => {
      setLoading(true);
      const result = await recipesApi.getSavedRecipes();

      if (result.success && result.recipes) {
        setRecipes(result.recipes);
      } else {
        logger.error(`Failed to load saved recipes: ${result.error}`);
      }
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <Link
              to="/explore"
              className="inline-flex items-center gap-2 bg-accent-orange hover:bg-[#E64A19] text-white px-6 py-2.5 rounded-full font-semibold transition-all shadow-lg"
            >
              <span className="material-symbols-outlined text-lg">search</span>
