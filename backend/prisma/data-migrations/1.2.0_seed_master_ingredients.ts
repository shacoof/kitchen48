/**
 * Data Migration: Seed Master Ingredients
 *
 * Database Version: 1.2.0
 *
 * This migration:
 * 1. Clears all existing master_ingredients data
 * 2. Inserts the cleaned ingredients list (4,599 valid ingredients)
 *
 * Source: misc/ingredients_cleaned.txt
 * Original source had 4,999 entries with invalid items removed:
 * - Dates, prices, store names
 * - Recipe labels (filling:, sauce:, etc.)
 * - Measurement-prefixed items (lg. onion, ml milk, etc.)
 * - Non-food items (scissors, parchment paper, etc.)
 * - Corrupted special character entries
 */
...
export const description = 'Clear and reseed master_ingredients with cleaned data';
...
export async function run(): Promise<void> {
  console.log('Starting master_ingredients migration...');
  console.log(`Total ingredients to insert: ${INGREDIENTS.length}`);

  // Use transaction for safety
  await prisma.$transaction(async (tx) => {
    // Step 1: Clear existing data
    console.log('Clearing existing master_ingredients...');
    const deleted = await tx.masterIngredient.deleteMany({});
    console.log(`  Deleted ${deleted.count} existing records`);

    // Step 2: Insert new data in batches
    console.log('Inserting cleaned ingredients...');
    ...
    for (let i = 0; i < INGREDIENTS.length; i += BATCH_SIZE) {
      ...
      await tx.masterIngredient.createMany({
        data: batch.map(name => ({ name })),
        skipDuplicates: true
      });
      
      inserted += batch.length;
      if (inserted % 500 === 0 || inserted === INGREDIENTS.length) {
        console.log(`  Inserted ${inserted}/${INGREDIENTS.length} ingredients`);
      }
    }
  }, {
    timeout: 60000 // 60 second timeout for large operation
  });

  // Verify
  const count = await prisma.masterIngredient.count();
  console.log(`Migration complete. Total ingredients in database: ${count}`);
}
