...
// Languages to seed: value (ISO 639-1) → English label → Hebrew label
...
async function seedLanguages() {
  console.log('Seeding Languages LOV...');
  ...
  console.log(`  List type "Languages": ${listType.id}`);

  // Create language values and their Hebrew translations
  for (let i = 0; i < supportedLanguages.length; i++) {
    const lang = supportedLanguages[i];

    // Upsert the list value
    ...
    // Upsert English translation (so translations table has both)
    await prisma.listValueTranslation.upsert({
      where: {
        listValueId_language: {
          listValueId: listValue.id,
    ...
    console.log(`  Language "${lang.value}" (${lang.label} / ${lang.he}): OK`);
  }
